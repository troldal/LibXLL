...
struct XllFnVariantImpl<std::index_sequence<Is...>> {
    using type = std::variant<XllFnN<Is>...>; ///< `std::variant` of all typed function-pointer types up to arity `sizeof...(Is) - 1`.
};
...
inline constexpr std::size_t function_arity_v =
    []<typename R, typename... Args>(R(*)(Args...)) -> std::size_t {
        return sizeof...(Args);
    }(F{});
...
struct Function {
     ...
     */
    [[nodiscard]] xll::Any operator()(const std::vector<xll::Any>& args,
