...
namespace xll
{
    ...
    public:
        ...
        //
        // An Expected<TValue, TError> is treated as a range of zero or one
        // elements.  If has_value() is true the range contains the single
        // TValue element; otherwise it is empty.  This mirrors the C++26
        // iterator support proposed for std::expected / std::optional.
        // ------------------------------------------------------------------

        /**
         * @brief Iterator for xll::Expected treated as a single-element range.
         *
         * Satisfies std::contiguous_iterator when engaged (pointer-based), so the
         * resulting range automatically satisfies std::ranges::contiguous_range.
         * When the iterator is constructed from a null pointer it serves as the
         * end sentinel.
         */
        class iterator
        {
        ...
        /**
         * @brief Const iterator for xll::Expected treated as a single-element range.
         */
        ...
        /**
         * @brief Returns true if the range is empty (no value).
         */
        [[nodiscard]]
         ...
         *
         * Useful for data() access in contiguous-range algorithms.
         */
        [[nodiscard]]
...
// Opt-in to std::ranges view / borrowed-range for xll::Expected.
// An Expected is treated as an owning range of 0 or 1 elements, so it is
// a view (cheap to copy, O(1) size) but NOT a borrowed range (iterators
// may dangle after the Expected is destroyed).
namespace std::ranges
{
    template<typename TValue, typename TError, typename TPolicy>
    inline constexpr bool enable_view<xll::Expected<TValue, TError, TPolicy>> = true;
}    // namespace std::ranges
