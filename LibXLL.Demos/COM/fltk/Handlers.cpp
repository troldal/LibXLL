...
auto onConnection = com::OnConnection(
    ...
    {
        std::cerr << "[xlCOM-Fltk] OnConnection fired. "
...
auto onButtonClicked = com::DispatchCallback<"OnButtonClicked">(
    [](DISPPARAMS*, VARIANT*) -> HRESULT
    {
        ...
        com::String macroName(L"FLTK.STATUS");
        VARIANT arg  = {};
        arg.vt       = VT_BSTR;
...
auto onRibbonLoad = com::DispatchCallback<"OnRibbonLoad">(
    [](DISPPARAMS* pDispParams, VARIANT*) -> HRESULT
    {
        ...
        {
            std::cerr << "[xlCOM-Fltk] OnRibbonLoad fired — IRibbonUI captured.\n";
        }
        return S_OK;
    });
    ...
    [](DISPPARAMS*, VARIANT*) -> HRESULT
    {
        ...
        if (g_taskPane)
        {
            ...
            if (SUCCEEDED(hr) && visible)
            {
                ...
                std::cerr << "[xlCOM-Fltk]   deleting visible pane\n";
            ...
            std::cerr << "[xlCOM-Fltk]   pane hidden/stale — deleting and recreating\n";
            TaskPane_Delete(g_taskPane);
            g_taskPane->Release();
            g_taskPane = nullptr;
        }
        ...
        if (!g_ctpFactory)
        {
            std::cerr << "[xlCOM-Fltk]   no CTP factory available\n";
            return E_FAIL;
        }
        ...
        if (hr == DISP_E_EXCEPTION)
        {
            if (excep.bstrDescription)
                fprintf(stderr, "[xlCOM-Fltk] CreateCTP failed: %ls\n", excep.bstrDescription);
        ...
        if (SUCCEEDED(hr) && result.vt == VT_DISPATCH && result.pdispVal)
        {
            ...
            std::cerr << "[xlCOM-Fltk]   pane created, SetVisible hr=0x"
                      << std::hex << hr << std::dec << '\n';
        }
        VariantClear(&result);
