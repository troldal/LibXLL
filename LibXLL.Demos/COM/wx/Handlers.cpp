// ---------------------------------------------------------------------------
// Content type — wxWidgets
// ---------------------------------------------------------------------------

#include "WxTaskPane.hpp"
...
struct WxPaneTraits
{
    static constexpr CLSID   clsid        = detail::parseGUID("7C2D4F8D-3E1B-4A5C-9D6E-8F0B2A1C3D5E");
    static constexpr wchar_t progId[]     = L"xlCOM.Wx.TaskPane";
    static constexpr wchar_t friendlyName[] = L"xlCOM wxWidgets Task Pane Control";
};
...
template class TaskPaneControl<WxTaskPane, WxPaneTraits>;
template class TaskPaneControlFactory<WxTaskPane, WxPaneTraits>;
...
static const bool s_taskPaneHooked =
    detail::registerTaskPaneHooks<WxTaskPane, WxPaneTraits>();
...
static const com::AddIn s_addin(
    "1E0739E3-A1B5-4AB4-953C-2D8959196A13",
    L"xlCOM.Wx.Connect",
    L"xlCOM wxWidgets",
    L"xlCOM Excel COM Add-in (wxWidgets)"
);
...
auto onConnection = com::OnConnection(
    ...
    {
        std::cerr << "[xlCOM-Wx] OnConnection fired. "
...
auto onDisconnection = com::OnDisconnection(
    ...
    {
        std::cerr << "[xlCOM-Wx] OnDisconnection fired. "
...
auto onRibbonLoad = com::DispatchCallback<"OnRibbonLoad">(
    [](DISPPARAMS* pDispParams, VARIANT*) -> HRESULT
    {
        ...
        {
            std::cerr << "[xlCOM-Wx] OnRibbonLoad fired — IRibbonUI captured.\n";
        }
        return S_OK;
    });
...
auto onTaskPaneClicked = com::DispatchCallback<"OnTaskPaneClicked">(
    [](DISPPARAMS*, VARIANT*) -> HRESULT
    {
        std::cerr << "[xlCOM-Wx] OnTaskPaneClicked\n";
        ...
        if (g_taskPane)
        {
            ...
            if (SUCCEEDED(hr) && visible)
            {
                ...
                std::cerr << "[xlCOM-Wx]   deleting visible pane\n";
            ...
            std::cerr << "[xlCOM-Wx]   pane hidden/stale — deleting and recreating\n";
            TaskPane_Delete(g_taskPane);
            g_taskPane->Release();
            g_taskPane = nullptr;
        }
        ...
        if (!g_ctpFactory)
        {
            std::cerr << "[xlCOM-Wx]   no CTP factory available\n";
            return E_FAIL;
        }
        ...
        args[2].bstrVal = SysAllocString(WxPaneTraits::progId);
        ...
        if (hr == DISP_E_EXCEPTION)
        {
            if (excep.bstrDescription)
                fprintf(stderr, "[xlCOM-Wx] CreateCTP failed: %ls\n", excep.bstrDescription);
        ...
        if (SUCCEEDED(hr) && result.vt == VT_DISPATCH && result.pdispVal)
        {
            ...
            std::cerr << "[xlCOM-Wx]   pane created, SetVisible hr=0x"
                      << std::hex << hr << std::dec << '\n';
        }
        VariantClear(&result);
